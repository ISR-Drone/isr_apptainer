...
INSTANCE_NAME="mrs_uav_instance"
...
# Start instance instead of one-off run
$EXEC_CMD apptainer instance start \
  $NVIDIA_ARG \
  $OVERLAY_ARG \
  $CONTAINED_ARG \
  $WRITABLE_ARG \
...
# If you want an interactive shell immediately:
apptainer exec instance://$INSTANCE_NAME /bin/bash -c "$CMD"